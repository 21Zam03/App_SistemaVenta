...
public class Pdf {
    ...
    public String CrearPdf(List<DetalleVenta> detalle) {
        PdfWriter writer = null;
        try {
            //Ubicacion del archivo.
            File file = new File("C:/Zam Projects/App_SistemaVenta/PDFs/"+"000"+detalle.get(0).getVenta().getIdVenta()+".pdf");
            ...
            try ( Document documento = new Document(docPdf);) {
                Image imagen = null;
                try {
                    imagen = new Image(ImageDataFactory.create("C:/Zam Projects/App_SistemaVenta/src/main/java/com/zam/img/logo_negro.png"));
                ...
                Paragraph p2 = new Paragraph("Codigo de venta: "+detalle.get(0).getVenta().getIdVenta()+"\n"
                        + "Fecha: "+detalle.get(0).getVenta().getFecha());
                ...
                tablaCliente.addCell(new Cell().add(new Paragraph(detalle.get(0).getVenta().getCliente().getDni())).setBorder(Border.NO_BORDER));
                tablaCliente.addCell(new Cell().add(new Paragraph(detalle.get(0).getVenta().getCliente().getNombre())).setBorder(Border.NO_BORDER));
                tablaCliente.addCell(new Cell().add(new Paragraph(detalle.get(0).getVenta().getCliente().getTelefono())).setBorder(Border.NO_BORDER));
                tablaCliente.addCell(new Cell().add(new Paragraph(detalle.get(0).getVenta().getCliente().getDireccion())).setBorder(Border.NO_BORDER));
                ...
                for (int i=0; i<detalle.size(); i++) {
                    double precio_cantidad = detalle.get(i).getPrecio()*detalle.get(i).getCantidad();
                    total = precio_cantidad + total;                    
                    Table pro = new Table(4);
                    pro.addCell(new Cell().add(new Paragraph(detalle.get(i).getProducto().getNombre())).setBorder(Border.NO_BORDER).setWidth(250));
                    pro.addCell(new Cell().add(new Paragraph(String.valueOf(detalle.get(i).getCantidad()+" u"))).setWidth(88).setBorder(Border.NO_BORDER));
                    pro.addCell(new Cell().add(new Paragraph("S/"+String.valueOf(detalle.get(i).getPrecio()))).setWidth(85).setBorder(Border.NO_BORDER));
                    pro.addCell(new Cell().add(new Paragraph("S/"+String.format("%.2f",(detalle.get(i).getPrecio()*detalle.get(i).getCantidad())))).setBorder(Border.NO_BORDER));
        ...
        String nombre = "000"+detalle.get(0).getVenta().getIdVenta()+".pdf";
        this.abrirPdf(nombre);
        return nombre;
    }
    
    public void abrirPdf(String nombre) {
        File pdf = new File("C:/Zam Projects/App_SistemaVenta/PDFs/"+nombre);
        if (pdf.exists()) {
            try {
